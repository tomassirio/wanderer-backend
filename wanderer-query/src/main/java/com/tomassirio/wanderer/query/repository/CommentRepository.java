...
public interface CommentRepository extends JpaRepository<Comment, UUID> {
    ...
    /**
     * Find all top-level comments for a trip (comments with no parent). 
     * Uses EntityGraph to efficiently load user and reactions data.
     */
    @EntityGraph(attributePaths = {"user", "commentReactions", "commentReactions.user"})
    @Query("SELECT c FROM Comment c WHERE c.trip.id = :tripId AND c.parentComment IS NULL ORDER BY c.timestamp DESC")
    List<Comment> findTopLevelCommentsByTripId(@Param("tripId") UUID tripId);

    /**
     * Find all top-level comments for a trip with pagination.
     * Uses EntityGraph to efficiently load user and reactions data.
     */
    @EntityGraph(attributePaths = {"user", "commentReactions", "commentReactions.user"})
    @Query(
            value = "SELECT c FROM Comment c WHERE c.trip.id = :tripId AND c.parentComment IS NULL",
            countQuery = "SELECT COUNT(c) FROM Comment c WHERE c.trip.id = :tripId AND c.parentComment IS NULL")
     ...
     */
    @EntityGraph(attributePaths = {"user", "commentReactions", "commentReactions.user", "replies", "replies.user"})
