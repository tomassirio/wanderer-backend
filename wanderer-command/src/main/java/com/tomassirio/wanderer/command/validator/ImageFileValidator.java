...
/**
 * Validator for uploaded image files. Validates file format, size, and content type for profile
 * pictures and other image uploads.
 *
 * @author tomassirio
 * @since 0.10.5
 */
    ...
    private void validateFileSize(MultipartFile file) {
        if (file.getSize() > MAX_FILE_SIZE) {
            throw new IllegalArgumentException(
                    String.format(
                            "File size (%d bytes) exceeds maximum allowed size of 5MB",
                            file.getSize()));
        }
    }
