 ...
 * 
 * <p>Strategies are tried in order until one successfully handles the authentication.
 * The order matters: TokenAuthenticationStrategy is tried first, then AnonymousAuthenticationStrategy
 * as a fallback for guest users.
 */
@Slf4j
@Service
        ...
        for (WebSocketAuthenticationStrategy strategy : strategies) {
            if (strategy.canHandle(session)) {
                ...
                if (result.isAuthenticated()) {
                    log.debug("Session {} authenticated using {} (anonymous: {})",
                            session.getId(), 
