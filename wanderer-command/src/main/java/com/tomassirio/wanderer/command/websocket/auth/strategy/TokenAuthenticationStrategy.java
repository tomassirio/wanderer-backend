 ...
 * 
 * <p>This strategy attempts to extract and validate a JWT token from the
 * WebSocket query parameters. If successful, the user is authenticated.
 */
@Slf4j
@Component
...
public class TokenAuthenticationStrategy implements WebSocketAuthenticationStrategy {
    ...
    public AuthenticationResult authenticate(WebSocketSession session) {
        try {
            ...
            log.debug("Token authentication successful for session: {}, userId: {}", 
                    session.getId(), userId);
            return AuthenticationResult.authenticated(userId);
            
        } catch (Exception e) {
            log.warn("Token authentication failed for session {}: {}", 
